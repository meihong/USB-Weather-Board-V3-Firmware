...
volatile unsigned long raintime, rainlast, raininterval, rainserial, rainpulsetime, rainpulseinterval, rain;
...
// activated by the magnet and reed switch in the rain gauge, attached to input D2
{
  if (digitalRead(RAIN) == LOW)
  {
    ...
    if (rainserial != 1 && raininterval > 100) {
      rainserial    = 1;
...
// loops forever after setup() ends
{
  ...
  switch (data_format)
  {
    ...
    case CSV: // data_format: comma-separated values
    {
      ...
      Serial.print("$");
      printComma();
      Serial.print(SHT15_temp,2);
      printComma();
      Serial.print(SHT15_humidity,2);
      printComma();
      Serial.print(SHT15_dewpoint,2);
      printComma();
      ...
      printComma();
      Serial.print(rainpulseinterval);
      printComma();
      Serial.print("*");
      Serial.println();
    }
...
// provide the user a way to modify settings, and save those settings to EEPROM to survive reboot / shutdown
{
  ...
  Serial.print(version_major,DEC); Serial.print(".");  Serial.print(version_minor,DEC); Serial.println(" dame6");
